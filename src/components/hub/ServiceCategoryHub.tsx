...
export function ServiceCategoryHub() {
  ...
  return (
    <div className="space-y-3 w-full max-w-full">
      {/* Streamlined Top Header & Actions */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-2.5 p-2.5 sm:p-3 rounded-2xl bg-white border border-slate-200/90 shadow-2xs">
        <div className="flex items-center gap-2.5">
          <div className="p-1.5 sm:p-2 rounded-xl bg-purple-600 text-white shadow-xs shadow-purple-200 shrink-0">
        ...
        <div className="flex items-center gap-1.5 flex-wrap w-full lg:w-auto justify-start sm:justify-end text-[11px]">
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-700 font-semibold flex items-center gap-1 border border-slate-200/90 shadow-2xs transition-colors"
          >
          ...
          {activeTab === 'categories' && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white font-bold shadow-xs shadow-purple-200 flex items-center gap-1 transition-all"
            >
      ...
      {/* Primary 10 Navigation Tabs without ANY Browser Scrollbars */}
      <div
        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] transition-all flex items-center gap-1 shrink-0 border ${
                isActive
                  ? 'bg-purple-50/90 border-purple-400 text-purple-700 font-extrabold shadow-2xs'
                  : 'bg-white text-slate-600 border-slate-200/90 hover:text-slate-900 hover:bg-slate-50 font-medium'
              }`}
            >
              <Icon className={`w-3 h-3 ${isActive ? 'text-purple-600' : 'text-slate-400'}`} />
              <span className="whitespace-nowrap">{t.label}</span>
              {t.count !== undefined && (
                <span className={`text-[10px] font-mono px-1 py-0.1 rounded ${
                  isActive ? 'bg-purple-200/70 text-purple-900 font-bold' : 'bg-slate-100 text-slate-500'
                }`}>
                  {t.count}
                </span>
              )}
