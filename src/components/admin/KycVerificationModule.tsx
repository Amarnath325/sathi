      ...
      {kycViewMode === 'table' ? (
            ...
            <table className="w-full text-left border-collapse text-xs text-slate-300">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-950 text-[10px] font-mono text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="py-3.5 px-4 font-bold">Safety Tier</th>
                  <th className="py-3.5 px-4 font-bold">AI OCR & Liveness</th>
                  <th className="py-3.5 px-4 font-bold">Police BGV</th>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          <img
                            ...
                            className="w-9 h-9 rounded-xl object-cover border border-purple-500/30 shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
                            title="Click to view full selfie"
                          />
                          <div className="min-w-0">
                            <span className="font-extrabold text-white text-xs block truncate">{doc.userName}</span>
                            <span className="text-[10px] text-slate-400 font-mono block truncate">{doc.userEmail}</span>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex flex-col gap-0.5 text-[10px] font-mono">
                          <span className="text-purple-300 flex items-center gap-1">
                            <Sparkles className="w-3 h-3 text-purple-400" /> OCR: {doc.ocrData?.confidenceScore || 98.6}%
                          </span>
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {doc.status !== 'APPROVED' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold transition-all"
                            >
                              Approve
                            </button>
                          )}
                          {doc.status !== 'REJECTED' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-rose-600/20 text-rose-400 text-[11px] font-bold transition-all"
                            >
                              Reject
                            </button>
                          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          {displayedDocs.length === 0 ? (
            ...
            displayedDocs.map((doc) => (
              <div key={doc.id} className="p-3 rounded-2xl bg-slate-900 border border-slate-800 space-y-2 shadow-md hover:border-purple-500/40 transition-all flex flex-col justify-between">
                <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 gap-1">
                  <div className="min-w-0">
                    <h4 className="font-bold text-white text-xs flex items-center gap-1 truncate">
                      <span className="truncate">{doc.userName}</span>
                      {doc.status === 'APPROVED' && <CheckCircle2 className="w-3 h-3 text-emerald-400 shrink-0" />}
                    </h4>
                    <p className="text-[9px] text-slate-400 font-mono truncate">{doc.userEmail}</p>
                  </div>

                  <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-extrabold border shrink-0 ${
                    doc.status === 'APPROVED'
                      ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40'
                      : doc.status === 'REJECTED'
                      ? 'bg-rose-500/20 text-rose-300 border-rose-500/40'
                ...
                <div className="grid grid-cols-2 gap-1 text-[9px]">
                  <div className="p-1.5 rounded-lg bg-slate-950 border border-slate-800/80 min-w-0">
                    <span className="text-[8px] text-slate-500 font-mono uppercase block truncate">Doc Type</span>
                    <p className="font-bold text-purple-400 truncate text-[10px]">{doc.type.replace(/_/g, ' ')}</p>
                  </div>
                  <div className="p-1.5 rounded-lg bg-slate-950 border border-slate-800/80 min-w-0">
                    <span className="text-[8px] text-slate-500 font-mono uppercase block truncate">ID Number</span>
                    <p className="font-mono font-bold text-white truncate text-[10px]">{doc.documentNumber}</p>
                  </div>
                </div>
                ...
                <div 
                  onClick={() => setInspectingDoc(doc)}
                  className="relative rounded-xl overflow-hidden border border-slate-800/80 group h-24 bg-slate-950 flex items-center justify-center p-1 shadow-inner cursor-pointer hover:border-purple-500/50 transition-all"
                >
                  <img
                    src={doc.fileUrl}
                    alt="KYC Document"
                  ...
                  <div className="absolute top-1 left-1 flex flex-col gap-0.5">
                    <span className="px-1.5 py-0.5 rounded bg-purple-950/90 text-purple-300 text-[8px] font-mono font-bold border border-purple-800 flex items-center gap-0.5">
                      <Sparkles className="w-2.5 h-2.5 text-purple-400" /> {doc.ocrData?.confidenceScore || 98.6}%
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 pt-1 border-t border-slate-800/80">
                  <button
                    onClick={() => setInspectingDoc(doc)}
                    className="flex-1 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] font-bold"
                  >
                    Inspect
                  </button>
                  {doc.status !== 'APPROVED' && (
